...
Iteration 22 已完成成本报告与脱敏审计导出：`cost-report` 在保留期内按带时区 ISO-8601 的 UTC 规范化窗口、任务 ID 和 Provider ID 过滤模型完成事件，输出总计、按任务和按 Provider 的调用、Token、耗时及估算微美元/美元。窗口固定为起始包含、结束不包含；报告显式区分有成本估算与缺少估算的历史调用，避免把数据缺失静默解释为零成本。

逐调用明细默认关闭，只有 `--include-calls` 才输出事件号、时间、任务/步骤、模型元数据、Token、耗时和估算成本；原始 Prompt、Payload、查询、证据和回答仍不进入事件或报告。下一增量应建立供应商账单导入与差异对账契约，或优先扩展 Token/并发配额控制；在真实评测显示规划质量成为瓶颈前，Planner 和多 Agent 继续保持未启动。

Iteration 23 已完成严格供应商账单导入与成本差异对账：`provider_billing_statement_v1` 将一个 Provider、UTC 规范化计费周期、USD 微美元总额和唯一模型明细绑定为无扩展字段的输入契约；总额必须等于模型行金额，规范化文档 SHA-256 进入报告，但本地导入路径、账户和支付信息不进入输出。`billing-reconcile` 按同一 Provider/周期/模型对齐本地估算，输出金额、Token、模型存在性和估算完整性差异。

默认容差取 100 微美元和账单金额 5% 的较大值；任一模型或总额超差、模型映射缺失、存在本地调用无成本估算时返回 `3`，非法输入返回 `2`。该能力仍不是供应商原始账单解析器，也未覆盖税费、折扣、共享费用和外汇。下一增量建议优先扩展 Provider Token/并发配额控制，随后再根据真实账单样本增加受审适配器与持久化对账历史。

Iteration 24 已完成 Provider Token 与并发配额控制：模型使用预计输入加最大输出 Token 进行调用前预留，Embedding 使用可配置的单输入估算逐个真实批次预留；请求、Token 和并发状态在同一 SQLite 数据库中跨进程共享。Token 采用平滑预约且拒绝超过单分钟容量的单次估算，并发控制使用有 TTL 的租约，正常响应、异常与自适应拆批均在 `finally` 边界释放，进程中断留下的租约可在过期后回收。

新配置项均保持向后兼容：未声明 `tokens_per_minute` 或 `max_concurrent_requests` 时不启用相应限制。当前 Token 配额属于安全侧调用前估算，尚未使用响应中的实际 Usage 回补；并发租约也不解析供应商动态限流头。下一增量应以真实 Provider 行为和受审样本为依据选择动态配额/Usage 校正或账单适配器与长期对账历史，Planner 和多 Agent 继续等待评测证据。

Iteration 25 已完成共享动态冷却与实际 Usage 校正：HTTP `Retry-After`、OpenAI-compatible 请求/Token 剩余与重置头、标准 RateLimit 头在剩余额度明确为零时转换为 Provider 冷却，并以“只延长、不缩短”方式写入共享 SQLite。配额预约后、真正发起网络请求前会再次读取冷却，覆盖其他进程稍后收到 429 的竞态；无法明确解释的响应头保持忽略。

每次 Token 预约现有唯一记录，成功模型或 Embedding 响应若提供完整 Usage，则以事务幂等校正估算：低估一定追加，高估仅在没有后续预约时退回，晚到的退回保持保守以保护已经排队的进程。重复相同 Usage 不会重复修改额度，不同 Usage 则拒绝。当前实现仍不是所有供应商的配额解析器，未覆盖组织级共享配额、专有滑动窗口和失败响应中的实际 Usage。下一增量应优先增加脱敏配额观测指标，或在获得受审真实样本后建设专用适配器与长期对账历史。

Iteration 26 已完成 Provider 配额脱敏观测与回归门禁：共享限流器记录容量/并发等待、实际延长的冷却和首次 Token 校正，聚合层输出等待分位数、冷却强度以及 Token 估算绝对误差。事件契约只允许 Provider ID、配额种类和数值，不保存 HTTP 头、请求/响应正文、预约 ID 或绝对冷却时间；观测存储或校验故障保持 best-effort，不影响调用结果。

观测质量门禁新增 Provider 配额 P95 等待、冷却次数和 Token 估算绝对误差上限。旧版报告仍可生成原指标候选，但新旧指标集合不同的报告会被明确拒绝比较。下一增量应从受审真实 Provider 样本校准这些阈值并建设专用配额适配器，或扩展自然语言蕴含和真实用户反馈评测；Planner 与多 Agent 继续等待明确的评测瓶颈证据。
